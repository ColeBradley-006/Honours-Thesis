...
MatrixXf advecRHS(int Np, int Nfp, int Nfaces, int K, VectorXi vmapM, VectorXi vmapP, MatrixXf nx, float a, MatrixXf u, int mapI, int vmapI, int mapO, MatrixXf Dr, MatrixXf rx, MatrixXf Lift, MatrixXf Fscale, float localTime){
    float alpha = 0;
...
int main() {
    ...
    for (int nl = 1; nl < 6; nl ++){
        int N = nl;
        int elements[8]= {3,6,12,24,48,96};
        for (int i = 0; i < 6; i ++){
            tuple<int, VectorXf, int, MatrixXd> mesh = meshGen(0.0,4*pi,elements[i]);
            int Nv = get<0>(mesh);
            int K = get<2>(mesh);
            ...
            for (int t=0; t < Nsteps; t++){
                for (int i = 0; i < 5; i++){
                    ...
                    MatrixXf rhsu = advecRHS(Np, Nfp, Nfaces, K, vmapM, vmapP, nx, a, u, mapI, vmapI, mapO, Dr, rx, lift, Fscale, localTime);
            ...
            fstream myfile;
            myfile.open( "P" + to_string(nl) + "_E" + to_string(i) + "_DGout.txt",fstream::out);
            for (int n = 0; n < Np; n++){
                for (int j = 0; j < K; j++){
                    myfile << u(n,j) << "\t";
                }
            ...
            fstream xfile;
            xfile.open( "X_P" + to_string(nl) + "_E" + to_string(i) + "_DGout.txt",fstream::out);
            for (int n = 0; n < Np; n++){
                for (int j = 0; j < K; j++){
                    xfile << x(n,j) << "\t";
                }
