...
MatrixXf advecRHS(int Np, int Nfp, int Nfaces, int K, VectorXi vmapM, VectorXi vmapP, MatrixXf nx, float a, MatrixXf u, int mapI, int vmapI, int mapO, MatrixXf Dr, MatrixXf rx, MatrixXf Lift, MatrixXf Fscale){
    ...
    */ 
    ...
    int row = mapO%Nfaces;
    int col = floor(mapO/Nfaces);
    du(0,0) = (u(0,0) - u(row,col)) * (a*nx(0,0) - (1-alpha)*abs(a*nx(0,0)))/2;
    du(row, col) = (u(row,col) - u(0,0)) * (a*nx(row,col) - (1-alpha)*abs(a*nx(row,col)))/2;;
...
int main() {
    ...
    int N = 8;
    int elements[4]= {10,20,40,80};
    for (int i = 0; i < 4; i ++){
        tuple<int, VectorXf, int, MatrixXd> mesh = meshGen(0.0,2*pi,elements[i]);
        int Nv = get<0>(mesh);
        int K = get<2>(mesh);
        ...
        fstream myfile;
        myfile.open(to_string(i) + "DGout.txt",fstream::out);
        for (int n = 0; n < Np; n ++){
            for (int j = 0; j < K; j++){
                myfile << u(n,j) << "\t";
            }
