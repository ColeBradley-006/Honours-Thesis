...
for i in range(len(errorData)):
    for j in range(len(errorData[i])):
        errorData[i][j] = math.log(errorData[i][j])

numPoints = [math.log(10),math.log(20),math.log(41),math.log(82)]

plt.plot(numPoints, errorData[0], label = "Upwind")
plt.plot(numPoints, errorData[1], label = "Lax")
plt.plot(numPoints, errorData[2], label = "Lax-Wendroff")
plt.plot(numPoints, errorData[3], label = "Leap-Frog")
plt.plot(numPoints, errorData[4], label = "MacCormack")
